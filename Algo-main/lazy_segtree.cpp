...
private:
	...
	std::vector<int> seg;
	std::vector<int> lazy;
	void push(int k) {//ここいじくればいろいろできそうだね
		if (k < sz) {
			lazy[k * 2] = max(lazy[k * 2], lazy[k]);
			lazy[k * 2 + 1] = max(lazy[k * 2 + 1], lazy[k]);
		}
        //if(lazy[k]>0){seg[k]=lazy[k];}でもok下のやつは
		seg[k] = max(seg[k],lazy[k]);//特にこことか,この場合はlazy[k]==0の可能性あるからこうしている
		lazy[k] = 0;
	}
	void update(int a, int b, int x, int k, int l, int r) {
		...
		if (r <= a || b <= l) return;
		if (a <= l && r <= b) {
			lazy[k] = x;//ここいじくればいろいろできそうだね足し算したりetc..
			push(k);
			return;
		}
